...
class InputEmbeddings(nn.Module):
    def __init__(self, d_model: int, vocab_size: int):
        super(self, InputEmbeddings).__init__()
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        pos_enc = pos_enc.unsqueeze(0) # Dim (1, seq_len, d_model)
...
class FeedForwardBlock(nn.Module):
    def __init__(self,  d_model: int, d_ff: int, dropout: float):
        super(self, FeedForwardBlock).__init__()
...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttention, 
        ...
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([nn.Dropout(dropout) for _ in range(3)])
